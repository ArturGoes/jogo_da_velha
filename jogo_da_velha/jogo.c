    ...
    case 1: {
      ...
      if (deu_certo) {
        if (opcao == 1) {
          ...
          printf("Opção inválida! O jogador 1 será (0) e o jogador 2 será (X) "
                 "por padrão.\n");
        }
      } else {
        printf("Opção inválida! O jogador 1 será (0) e o jogador 2 será (X) "
      ...
      do {
        ...
        do {
          ...
          deu_certo = scanf("%d", &linha);
          if (!deu_certo) {
            while (getchar() != '\n')
              ;
            linha = coluna = -1; // Forçar nova leitura
          } else if (linha < 0 || linha > 2) {
            printf("Posição inválida! Por favor, escolha outra posição.\n");
            continue;
          } else {
            deu_certo = scanf("%d", &coluna);
      ...
      printf("\nDigite 1 para jogar novamente, ou qualquer outra tecla para "
             "sair: ");
